...
{	
	...
	if (data[EL_BIT_FROM] == EL_MCU)
	{
		if (data[EL_BIT_TO] == EL_MAIN_DOOR && data[EL_BIT_OP] == EL_OP_MCU_SET_MAIN_DOOR)
		{
			show_main_door(data[EL_BIT_PARAM]);
		}

		if (data[EL_BIT_TO] = EL_SUB_DOOR && data[EL_BIT_OP] == EL_OP_MCU_SET_SUB_DOOR)
		{
			...
			else
			{
				...
				switch (char_exit)
				{
					case EL_CHAR_MAIN_DOOR_OPEN:
						...
						printf("prepare to open\n");
						send(dev, data);
						printf("finish to open\n");
						break;
